...
Firebase ti avvisa che le regole sono aperte a tutti — è normale per come abbiamo configurato l'app. Per un uso familiare privato non è un problema reale.

Se vuoi però puoi mettere regole più sicure che permettono solo lettura pubblica e scrittura solo da chi conosce l'URL del database. Sostituisci le regole con:
...
Questo avviso lo riceverai comunque perché Firebase preferisce l'autenticazione utente. Puoi ignorarlo tranquillamente — i tuoi dati sono prenotazioni familiari, non dati sensibili critici.

Vedo il problema — il dato €/notte c'è nel codice ma non appare perché è nascosto sotto il badge "Confermata". Sposto la riga €/notte sotto il totale in modo che sia sempre visibile:


App.jsx — GitHub Pages
Codice ∙ Versione 49 
Ora il €/notte appare in arancione grassetto sopra il badge "Confermata/Provvisoria". Carica su GitHub!

ripeto , quando clicco su lista vorrei vedere questi dati per notte , non si vedono ancora

Capisco — vuoi che cliccando sulla prenotazione nella lista si apra il pannello dettaglio (come nel calendario) con tutti i dati incluso €/notte. Ora nella lista le card non sono cliccabili.

Aggiungo il tap sulla card per aprire il pannello dettaglio:


App.jsx — GitHub Pages
Codice ∙ Versione 50 
Ora cliccando su qualsiasi prenotazione nella lista si apre il pannello dettaglio dal basso con tutti i dati: totale, netto, €/notte, date, ospiti, note. I pulsanti Modifica ed Elimina appaiono solo se sei admin. Carica su GitHub!

non voglio aprire la prenotazione voglio vedere il dato nella lista su ogni prenotazione in modo di vedere tutto facendo scorrere le prenotazioni

Ah capito! Vuoi vedere €/notte direttamente sulla card senza cliccare nulla. Il dato c'è già nel codice ma probabilmente non si vede per un problema di layout.

Guardando la foto che hai mandato, vedo che a destra c'è solo il totale e il netto — il €/notte dovrebbe apparire sotto ma forse non c'è spazio. Lo metto sotto il prezzo in modo più visibile:


App.jsx — GitHub Pages
Codice ∙ Versione 51 
Ora ogni card nella lista mostra una riga dedicata con sfondo arancione chiaro:

Media per notte — €xxx,xx · €xxx,xx netto

Visibile direttamente senza cliccare nulla. Carica su GitHub!

RIPETO VOGLIO VEDERE QUESTI DATI QUANDO APRO LISTA SENZA DOVER APRIRE NIENTE , SOLO FACENDO SCORRERE CON IL MOUSE

Guardo lo screenshot che hai mandato — il dato €/notte dovrebbe già essere visibile sulla card.

Prova a ricaricare il sito con Ctrl+Shift+R dopo aver caricato il codice aggiornato su GitHub. Lo vedi adesso?

FATTO MA NON SI VEDE , SE VOGLIO VEDERLO DEVO CLICCARE SULLA PRENOTAZIONE

Ah capisco! Il problema è che cliccando sulla card ora si apre il pannello dettaglio invece di mostrare i dati inline. Rimuovo il click dalla card e lascio i dati visibili direttamente:


App.jsx — GitHub Pages
Codice ∙ Versione 52 
Ora i dati €/notte sono visibili direttamente sulla card nella lista, senza dover cliccare nulla. Carica su GitHub e prova!
      ...
      <div style={{maxWidth:520,margin:"0 auto",paddingBottom:100}}>
              ...
              {upcomingCheckouts.map(b=>{
                ...
                return <div key={b.id} onClick={()=>setViewBooking(b)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"1px solid rgba(255,255,255,0.06)",cursor:"pointer"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();setCalExportOpen(b.id);}} style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:7,padding:"4px 8px",color:"#f0e6d3",cursor:"pointer",fontSize:11}}>📅</button>
                  </div>
                </div>;
              })}
            </div>}
        ...
        {view==="calendar"&&(
          <div style={{padding:"0 12px"}}>
            ...
            {calView==="month"?(
              <>
                <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:2}}>
                  {DAYS_SHORT.map(d=><div key={d} style={{textAlign:"center",fontSize:9,color:"#C8A96E",padding:"2px 0"}}>{d}</div>)}
                </div>
              ...
              <div>
                ...
                {(isAllSel()?apts:apts.filter(a=>Array.isArray(selApt)&&selApt.includes(a.id))).map(apt=>(
                  <div key={apt.id} style={{display:"grid",gridTemplateColumns:"38px repeat(7,1fr)",gap:2,marginBottom:2}}>
                    ...
                    {weekDays.map((d,i)=>{
                      const iso=toISO(d);
                      const bk=bookings.find(b=>b.apt===apt.id&&iso>=b.checkin&&iso<b.checkout);
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:7,marginTop:10}}>
              {apts.map(a=><div key={a.id} style={{display:"flex",alignItems:"center",gap:3,fontSize:10,color:"#ccc"}}><div style={{width:8,height:8,borderRadius:2,background:a.color}}/>{a.name}</div>)}
              <div style={{display:"flex",alignItems:"center",gap:3,fontSize:10,color:"#FF6B6B"}}><div style={{width:8,height:8,borderRadius:"50%",background:"#FF6B6B"}}/>Check-out</div>
              <div style={{display:"flex",alignItems:"center",gap:3,fontSize:10,color:"#4CAF8A"}}><div style={{width:8,height:8,borderRadius:"50%",background:"#4CAF8A"}}/>Check-in</div>
            </div>
          </div>
        )}
          ...
          <div style={{padding:"0 12px"}}>
            ...
            {listFilter!=="maint"&&<>
              ...
              {filteredBookings.map(b=>{
                const a=apts.find(x=>x.id===b.apt);
                ...
                return <div key={b.id} style={{background:"rgba(255,255,255,0.05)",borderRadius:11,padding:13,marginBottom:9,borderLeft:`4px solid ${a?a.color:"#888"}`,outline:soon?"1px solid rgba(255,107,107,0.3)":"none"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:a?a.color:"#888"}}>{a?.emoji} {a?.name}</div>
                      {b.platform&&<div style={{marginTop:4,display:"inline-block",padding:"2px 8px",borderRadius:10,background:platColor(b.platform)+"33",border:`1px solid ${platColor(b.platform)}66`,fontSize:10,color:platColor(b.platform),fontWeight:"bold"}}>{platEmoji(b.platform)} {platLabel(b.platform)}</div>}
                    </div>
                    <div style={{textAlign:"right"}}>
                      {prc>0&&<div style={{fontSize:17,fontWeight:"bold",color:"#4CAF8A"}}>€{fmtEur(prc)}</div>}
                      {prc>0&&<div style={{fontSize:10,color:"#7EC8E3"}}>~€{fmtEur(prc*taxMult)} netto</div>}
                  ...
                  {b.guests&&<div style={{fontSize:11,color:"#aaa",marginBottom:3}}>Ospiti: {b.guests}</div>}
          ...
          return <div style={{padding:"0 12px"}}>
            ...
            <div style={{background:"rgba(255,255,255,0.05)",borderRadius:11,padding:14,marginBottom:14,border:"1px solid rgba(255,255,255,0.08)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <button onClick={()=>setStatsYear(y=>y-1)} style={{...S.navBtn,width:24,height:24,fontSize:14}}>‹</button>
                  <div style={{fontSize:13,color:"#C8A96E",minWidth:36,textAlign:"center"}}>{statsYear}</div>
                  <button onClick={()=>setStatsYear(y=>y+1)} style={{...S.navBtn,width:24,height:24,fontSize:14}}>›</button>
                </div>
              </div>
              <div style={{display:"flex",gap:8,marginBottom:8}}>
                <div style={S.card}><div style={{fontSize:17,fontWeight:"bold",color:"#4CAF8A"}}>€{fmtEur(totRev)}</div><div style={{fontSize:9,color:"#999"}}>LORDO</div></div>
            ...
            {(()=>{
              ...
              return <div style={{background:"rgba(200,169,110,0.07)",borderRadius:11,padding:14,marginBottom:14,border:"1px solid rgba(200,169,110,0.35)"}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  <div style={S.card}><div style={{fontSize:16,fontWeight:"bold",color:"#C8A96E"}}>{allBks.length}</div><div style={{fontSize:9,color:"#999"}}>PREN. TOTALI</div></div>
                  <div style={S.card}><div style={{fontSize:16,fontWeight:"bold",color:"#C8A96E"}}>{allNts}</div><div style={{fontSize:9,color:"#999"}}>NOTTI TOTALI</div></div>
                </div>
              </div>;
            })()}
            <div style={{background:"rgba(255,255,255,0.04)",borderRadius:11,padding:14,marginBottom:14,border:"1px solid rgba(200,169,110,0.12)"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(12,1fr)",gap:3,marginBottom:8}}>
                {MONTHS_SHORT.map((m,i)=><div key={i} onClick={()=>setStatsMonth(statsMonth===i?null:i)} style={{textAlign:"center",fontSize:7,color:statsMonth===i?"#C8A96E":"#666",cursor:"pointer",fontWeight:statsMonth===i?"bold":"normal",borderBottom:statsMonth===i?"1px solid #C8A96E":"none"}}>{m}</div>)}
              </div>
              {statsMonth===null&&(()=>{
                ...
                return <div style={{borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:12}}>
                  {rows.map((d,i)=><div key={i} onClick={()=>setStatsMonth(i)} style={{display:"grid",gridTemplateColumns:"52px 1fr 36px 40px 52px",gap:4,padding:"5px 2px",borderRadius:6,cursor:"pointer",background:statsMonth===i?"rgba(200,169,110,0.1)":"transparent",marginBottom:1}}>
                    ...
                    <div style={{fontSize:11,color:d.bks>0?"#f0e6d3":"#444",textAlign:"right"}}>{d.bks||"—"}</div>
                    <div style={{fontSize:11,color:d.nts>0?"#C8A96E":"#444",textAlign:"right"}}>{d.nts||"—"}</div>
                    <div style={{fontSize:11,color:d.avg>0?"#FFB347":"#444",textAlign:"right"}}>{d.avg>0?d.avg.toFixed(1)+"n":"—"}</div>
                  </div>)}
                  <div style={{display:"grid",gridTemplateColumns:"52px 1fr 36px 40px 52px",gap:4,padding:"6px 2px",borderTop:"1px solid rgba(200,169,110,0.3)",marginTop:4}}>
                    <div style={{fontSize:10,color:"#C8A96E",fontWeight:"bold"}}>TOT.</div>
                    <div style={{fontSize:11,color:"#4CAF8A",textAlign:"right",fontWeight:"bold"}}>€{fmtEur(tot.rev)}</div>
                    <div style={{fontSize:11,color:"#f0e6d3",textAlign:"right",fontWeight:"bold"}}>{tot.bks}</div>
                    <div style={{fontSize:11,color:"#C8A96E",textAlign:"right",fontWeight:"bold"}}>{tot.nts}</div>
                    <div style={{fontSize:11,color:"#FFB347",textAlign:"right",fontWeight:"bold"}}>{tot.bks>0?(tot.nts/tot.bks).toFixed(1)+"n":"—"}</div>
                  </div>
                </div>;
              })()}
              {statsMonth!==null&&(()=>{
                ...
                return <div style={{borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:12}}>
                  ...
                  <button onClick={()=>setStatsMonth(null)} style={{...S.btn("transparent","#C8A96E44"),width:"100%",marginTop:6,padding:"7px 0",fontSize:12,color:"#999"}}>✕ Chiudi mese</button>
                </div>;
              })()}
            </div>
            {(()=>{
            ...
            {(()=>{
              ...
              return <div style={{background:"rgba(255,255,255,0.04)",borderRadius:11,padding:14,marginBottom:14,border:"1px solid rgba(200,169,110,0.12)"}}>
                ...
                {PLATFORMS.map(p=>{ const d=byP[p.id]; const pct=bksFilt.length>0?(d.count/bksFilt.length*100):0; return <div key={p.id} style={{marginBottom:12}}>
                  ...
                  <div style={{height:6,background:"rgba(255,255,255,0.06)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:p.color,borderRadius:3}}/></div>
                </div>;})}
              </div>;
            })()}
            {(()=>{
              ...
              return <div style={{background:"rgba(255,255,255,0.04)",borderRadius:11,padding:14,marginBottom:14,border:"1px solid rgba(200,169,110,0.12)"}}>
                ...
                {rankA.map((a,i)=>{ const pct=mxA>0?a.rev/mxA*100:0; return <div key={a.id} style={{marginBottom:9}}>
                  ...
                  <div style={{height:7,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:`linear-gradient(90deg,${a.color}66,${a.color}cc)`,borderRadius:4}}/></div>
                </div>;})}
              </div>;
            })()}
            {aptsShow.map(a=>{
              ...
              return <div key={a.id} style={{background:"rgba(255,255,255,0.04)",borderRadius:11,padding:13,marginBottom:9,borderLeft:`4px solid ${a.color}`}}>
                ...
                <div style={{height:5,background:"rgba(255,255,255,0.06)",borderRadius:3,marginBottom:10,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:a.color,borderRadius:3}}/></div>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><div style={{fontSize:10,color:"#999"}}>OCCUPAZIONE</div><div style={{fontSize:13,fontWeight:"bold",color:oc}}>{occ}%</div></div>
                <div style={{height:8,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden",marginBottom:10}}><div style={{height:"100%",width:occ+"%",background:oc,borderRadius:4}}/></div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                  <div style={S.card}><div style={{fontSize:13,fontWeight:"bold"}}>{bkA.length}</div><div style={{fontSize:9,color:"#999"}}>PREN.</div></div>
                  <div style={S.card}><div style={{fontSize:13,fontWeight:"bold",color:"#C8A96E"}}>{nA}</div><div style={{fontSize:9,color:"#999"}}>NOTTI</div></div>
                  <div style={S.card}><div style={{fontSize:13,fontWeight:"bold",color:"#4CAF8A"}}>€{fmtEur(rA)}</div><div style={{fontSize:9,color:"#999"}}>LORDO</div></div>
                </div>
              </div>;
            })}
          </div>;
        ...
        {view==="add"&&(
          <div style={{padding:"0 14px"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={handleSave} style={{...S.btn("#1a3a2a","#4CAF8A"),flex:2,padding:"12px 0",fontWeight:"bold"}}>{editId!==null?"Aggiorna":"Salva"}</button>
            </div>
          </div>
        )}
        ...
        {view==="addMaint"&&(
          <div style={{padding:"0 14px"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
              <Pill active={mForm.apt==="all"} color="#ffd580" onClick={()=>setMForm(f=>({...f,apt:"all"}))}>Tutta la villa</Pill>
              {apts.map(a=><Pill key={a.id} active={mForm.apt===a.id} color={a.color} onClick={()=>setMForm(f=>({...f,apt:a.id}))}>{a.emoji} {a.name}</Pill>)}
            </div>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{setEditMId(null);setMForm(emptyMForm());setView("list");}} style={{...S.btn("#1a1a2e","#888"),flex:1,padding:"12px 0"}}>Annulla</button>
              <button onClick={handleMSave} style={{...S.btn("#2a2a1a","#ffd580"),flex:2,padding:"12px 0",fontWeight:"bold"}}>{editMId!==null?"Aggiorna":"Salva"}</button>
            </div>
          </div>
        )}
          ...
          <div style={{padding:"0 14px"}}>
            ...
            {DEFAULT_APARTMENTS.map(a=>(
              <div key={a.id} style={{marginBottom:12}}>
                ...
                <input value={aptNames[a.id]??a.name} onChange={e=>setAptNames(n=>({...n,[a.id]:e.target.value}))} style={S.input}/>
              </div>
            ))}
            ...
            <div style={{display:"flex",gap:8,marginBottom:6}}>
              <div style={{flex:1}}><div style={{fontSize:10,color:"#999",marginBottom:5}}>Tasse (%)</div><input type="number" value={taxRate} onChange={e=>setTaxRate(e.target.value)} placeholder="21" min="0" max="100" style={S.input}/></div>
